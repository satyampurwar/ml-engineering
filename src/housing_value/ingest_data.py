...
optional arguments:
    ...
    --log-data LOG        file to store log data, default val in setup.cfg: logs/main.log if --no-console-log is opted
...
def filter_func(member, parent):
    """Filter function for extracting files from a tar archive.

    Parameters
    ----------
    member : object
        The TarInfo object representing a member of the archive.
    parent : str
        The parent directory to which the member is extracted (not used in this case).

    Returns
    -------
    member : object
        Returns the TarInfo object if it should be extracted, or None if it should be skipped.

    """
    # Check if the member's name starts with a dot (.)
    if member.name.startswith("."):
        return None  # Skip hidden files to prevent extraction

    # If the file does not meet the filtering criteria, allow it to be extracted
    return member  # Return the member for extraction
...
def fetch_housing_data(housing_url, housing_path):
    ...
    with tarfile.open(tgz_path, "r:gz") as housing_tgz:
        housing_tgz.extractall(path=housing_path, filter=filter_func)
        housing_tgz.close()
    return None
...
def save_split_data(df, processed_path=None, split_size=0.2):
    ...
    df.loc[:, df.select_dtypes(include=["int", "float"]).columns] = df.select_dtypes(
        include=["int", "float"]
    ).fillna(0)
    df.loc[:, df.select_dtypes(include=["int", "float"]).columns] = df.select_dtypes(
        include=["int", "float"]
...
if __name__ == "__main__":
    ...
    if not args.NOCONSOLE:
        ...
        if not args.LOG:
            dir_path = config["Default"]["log_directory"]
